...
test('operations assignment preserves readiness, authorization and exclusive custody', async (context) => {
  ...
  await context.test('unassigned queue contains only READY deliveries', async () => {
    ...
    assert.equal(body.deliveries.some((delivery) => delivery.id === ready.deliveryId), true);
    assert.equal(body.deliveries.some((delivery) => delivery.id === pending.deliveryId), false);
  });
  ...
  await context.test('assignment before READY is rejected without evidence', async () => {
    ...
    assert.equal(
      await prisma.auditLog.count({
        where: { aggregateType: 'Delivery', aggregateId: pending.deliveryId, action: 'AssignCourier' },
      }),
      0,
    );
  });

  await context.test('operations assigns once and a safe retry does not duplicate evidence', async () => {
    const ready = await createReadyOrder(baseUrl);
    const courierId = await createCourier();

    const assigned = await assignCourier(
      baseUrl,
      ready.deliveryId,
      courierId,
      1,
    ...
    const repeated = await assignCourier(
      baseUrl,
      ready.deliveryId,
      courierId,
      1,
    ...
    assert.equal(
      await prisma.auditLog.count({
        where: { aggregateType: 'Delivery', aggregateId: ready.deliveryId, action: 'AssignCourier' },
      }),
      1,
    );
    assert.equal(
  ...
  await context.test('stale version is rejected before assignment', async () => {
    ...
    const persisted = await prisma.delivery.findUniqueOrThrow({ where: { id: ready.deliveryId } });
  ...
  await context.test('one courier cannot own two active deliveries', async () => {
    ...
    assert.equal(
      await prisma.courierAssignment.count({ where: { courierId, active: true } }),
      1,
    );
    const secondDelivery = await prisma.delivery.findUniqueOrThrow({ where: { id: second.deliveryId } });
